...
public sealed class RecordOrderProcessedHandler : IRequestHandler<RecordOrderProcessed, Unit>
{
    private readonly IMongoCollection<OrderHistoryDocument> _collection;

    public RecordOrderProcessedHandler(IMongoDatabase database)
    {
        _collection = database.GetCollection<OrderHistoryDocument>("order_history");
    }

    public async Task<Unit> Handle(RecordOrderProcessed request, CancellationToken ct)
    {
        ...
        var update = Builders<OrderHistoryDocument>.Update
            ...
            .Set(x => x.OrderProcessedAt, request.OccurredAt);
